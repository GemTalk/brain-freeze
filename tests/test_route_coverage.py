...
`app.py` cannot be measured by a coverage tool. It imports `gemdb` and runs
inside GemStone under Grail, so it cannot be imported under CPython at all,
and `coverage.py` never sees the process that actually serves a request. The
57 in-database tests and the acceptance suite exercise it heavily -- and
nothing tells you what they miss. That was the largest blind spot in the
project when it was measured.

This is the honest substitute: read the routes out of `app.py`'s syntax tree,
read the tests and feature files, and require that each route is reached by
name. It is coarser than line coverage, and it answers the question that
...
def declared_routes():
    """(rule, handler name) for every @app.route in app.py."""
    with open(os.path.join(REPO, "app.py")) as handle:
...
class EveryRouteIsDriven(unittest.TestCase):
    ...
    def test_every_route_is_driven_by_a_test_or_a_scenario(self):
        ...
        self.assertEqual(
            ...
            "app.py cannot be measured by coverage.py, so this check is the "
            "only thing standing between a route and nobody noticing it "
            "broke." % "\n  ".join(missing))
