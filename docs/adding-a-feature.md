...
| the notebook | generated by `tools/make_notebook.py` — **never edit `brain-freeze.ipynb` by hand** | `gemdb tools/run_notebook_check.py` |
...
The notebook and the MCP answers are the two surfaces with no scenario. Both
are checked, and neither is checked by a browser: a notebook is run cell by
cell by its own runner, and the MCP answers are replayed over the real
transport against a freshly seeded book. Driving either through Playwright
would test somebody else's client.
