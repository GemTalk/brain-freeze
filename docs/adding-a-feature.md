...
**The acceptance suite** (`features/`) drives the real app in a real browser,
and the JSON surface with an HTTP client, leaving its evidence in
`artifacts/`. It needs its own virtualenv because it
carries a browser:
...
If you add a user-visible feature, it belongs in a `.feature` file, and if you
add a **route** you have no choice: a full run records every request it drives
and fails at the end naming any route no scenario reached. Running a single
feature file skips that check, so run the whole suite before you believe it.

Every scenario has to end in something a reader can look at — a screenshot, or
the payload for a surface that has no picture. A scenario that passes and
captures nothing fails the run.

The evidence is evidence of a run, not documentation: the app renders today's
...
| --- | --- | --- |
| the web app | `web/routes_html.py` renders `web/templates.py`; `web/app.py` wires it up | `tests/test_app.py`, `features/front_door.feature`, `features/quote.feature`, `features/claim.feature`, `features/refusals.feature` |
| the JSON API | `web/wire.py` serialises; `web/routes_api.py` routes it | `tests/test_api.py`, `features/json_api.feature` |
| the questionnaire | `web/forms.py` — asked by both surfaces, read by one function | `tests/test_api.py`, `tests/test_app.py`, `features/json_api.feature` |
| the MCP answers | `docs/mcp-questions.md`, generated by `gemdb tools/make_mcp_questions.py` | `python3 tools/refresh_mcp.py --verify` replays them over the transport; `features/every_surface_agrees.feature` runs the published snippets against the live book |
| a policy changing under a running app | `tools/lapse.py`, from a session of its own | `features/cross_surface.feature` |
| the notebook | generated by `tools/make_notebook.py` — **never edit `brain-freeze.ipynb` by hand** | `gemdb tools/run_notebook_check.py`, `features/every_surface_agrees.feature` |
| the dataset description | `docs/dataset-for-agents.md`, `docs/csv-schema.md` | by hand — nothing checks these |
| the seeded figures | `data/*.csv` | `tests/test_seed.py`, `tests/test_analysis.py`, `EXPECTED` in `tools/verify_book.py`, and the answers printed in `docs/mcp-questions.md` |
| the package boundary | `brainfreeze/` imports nothing Grail lacks | `tests/test_packaging.py` |

Every surface has a scenario. The notebook and the published agent answers
are the two a browser cannot reach, so `features/every_surface_agrees.feature`
drives them the way a person would — the notebook through its own runner, the
snippets by running them in a session of their own — and then requires the
answers to match the page and the payload.

What that feature deliberately does not check is the frozen figures printed in
`docs/mcp-questions.md`. Those describe a freshly seeded book, and the suite
has bought a policy and filed claims by the time it runs. Replaying them
against a fresh seed is `tools/refresh_mcp.py --verify`, over the real
transport.
