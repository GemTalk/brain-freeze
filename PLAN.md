...
| `findings/` | eight scripts that reproduce the Grail findings | done |
...
| `run_db_tests.py` | runs the whole suite inside the database | done |
| `brainfreeze/money.py` | money as `decimal.Decimal`, and the rounding Grail lacks | done |
| `redeploy.py` | makes the database run the source on disk | done |
| `brain-freeze.ipynb` | 19 cells; generated by `make_notebook.py` | done |
| `run_notebook_check.py` | runs its cells inside the database | done |
| `docs/mcp-questions.md` | 9 questions, answers generated by running them | done |
| `refresh_mcp.py` | keeps the MCP server current, and re-checks the promises | done |
| `verify_book.py` | reads the committed book back, from a session of its own | done |
| `tests/test_refresh_mcp.py` | 11 tests pinning the stamp, drift and promise parsing | done |
| the MCP transport check | — | **done 2026-09-09: 9 of 9 promises kept** |
