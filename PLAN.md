...
web/           the app: factory, routes, templates, questionnaire, serialiser
tools/         every command: seed, redeploy, verify, lapse, runners, generators
...
findings/      nine scripts that reproduce the Grail findings
mockups/       nine screens, an insurer sketch, and build_c.py that makes them
docs/          the PRD and everything written since
PLAN.md        this file
```

Neither `web/` nor `tools/` has an `__init__.py`, and neither may get one: a
committed package module is served from the database forever after, while a
...
| `brainfreeze/model.py` | `Book`, `Policyholder`, `Event`, `Claim`, `SavedQuote` | done |
| `brainfreeze/analysis.py` | the named questions an agent composes | done |
| `brainfreeze/money.py` | money as `decimal.Decimal`, and the rounding Grail lacks | done |
...
| `web/app.py` | the factory, the transaction beat, the server | done, **runs and serves** |
| `web/routes_html.py` | nine pages | done |
| `web/routes_api.py` | six JSON endpoints | done |
| `web/templates.py` | six templates and the stylesheet | done |
| `web/forms.py` | the questionnaire, and the one reader both surfaces use | done |
| `web/lookups.py` | finding the object a request is about, and the 404 | done |
| `web/wire.py` | model objects → JSON-ready dicts, money as exact strings | done |
| `tools/seed.py` | `data/` → objects in `gemdb.root`, with a smoke test | done, **verified against a real database** |
...
| `brain-freeze.ipynb` | 21 cells, 11 of them code; generated by `tools/make_notebook.py` | done |
| `docs/mcp-questions.md` | 9 questions, answers generated by running them | done |
| the MCP transport check | — | **done: 9 of 9 promises kept** |
| `README.md` | sequences CUJ-0→4, every command and output really run | done |
...
this state starts fine, runs Python fine, seeds fine, and passes the whole
model suite, because `brainfreeze/` is standard-library only and never
touches `re`. That
...
**These entries are dated records.** Each one says what was true on the day it
was written, and the figures in them are from that day -- test counts, route
counts, file names before things moved. They are not corrected afterwards,
because a work log that is edited to agree with the present is no longer
evidence of anything. What is current is the index at the top of this file,
and the README.
...
top-level `app.py` in the same tree took effect immediately. `run_db_tests.py`
...
### 3. The notebook — **DONE (2026-09-08)**

`brain-freeze.ipynb`, 19 cells. Generated by `make_notebook.py` rather than
...
FR-3.2 is indeed free: pick **GemDB** in the kernel picker, and that is the
connection step.

The strongest cell is the introspection one, and not for the reason expected.
A policyholder answers to **33 public names and stores 15**: `risk_tier`,
`total_paid` and `loss_ratio` are not fields that could drift out of step with
the data, they are questions the object answers. That is a sharper way to say
"no schema" than any amount of prose. (`asFloat` and `mro` show up in the full
list, leaking in from the Smalltalk side — worth a sentence, because it is
evidence these are real GemStone objects rather than Python in a wrapper.)

The chart is drawn in text. Grail has no matplotlib and the kernel renders
`text/plain` only, so `bar_chart` is ten lines in a cell — which is arguably
better here, since the ten lines are themselves part of the point.

### The refresh beat: the plan's script for it was wrong twice

The beat is real. A second session committed a policy and the first went on
reporting the old count indefinitely — the objects are shared *and* each
surface has its own transaction. But "call refresh, see it" does not work:

```
A: before                        902
A: needs_commit?                 True     <- merely having run code
A: after their commit, no action 902      <- stale, as promised
A: refresh() refused: refresh() would discard uncommitted changes
A: after abort()                 903
```

**`gemdb.refresh()` on its own will normally refuse in a notebook.** Not
because you changed data — Grail compiles your code into the database, so
running any cell leaves the session with uncommitted work and
`needs_commit()` returning `True`.

**And `gemdb.abort()` is the wrong fix**, though it does take a new view. It
discards this session's uncommitted work, which includes the functions defined
in earlier cells. Tested directly: after `abort()`, calling a helper defined
three cells earlier raises `a __main__ class does not understand
#'my_cell_helper:'`. A demo that aborts loses `bar_chart`.

**The recipe is `gemdb.commit()` then `gemdb.refresh()`.** Verified: the new
view arrives, and the earlier cell's helper still answers. The notebook says
all of this in prose beside the cell, because an evaluator will hit the refusal
whether or not it is scripted.

**Still to do:** the README has to sequence the beat as steps (file a claim in
the browser, re-run, see nothing, commit+refresh, see it), and loss ratio by
`policy_status` is still worth adding beside loss ratio by tier.
...
**The questions are shipped and verified (2026-09-08):**
`docs/mcp-questions.md`, nine of them, generated by `gemdb
make_mcp_questions.py` — which re-seeds and then *runs* every snippet, so the
answers cannot be typed in wrong. Regenerating after a data change is how the
demo's promises get caught drifting. (The first run reported 901 policies
because the app's tests had left claims behind; hence the re-seed.)

Two of them carry the demo rather than just answering:

- *"Why was CLM-001291 refused?"* returns the claim id, the reason, the event
  date, the lapse date **and** the in-force test — so the refusal is checkable
  rather than a stored string an agent has to trust. That is what CUJ-2 needs.
- *"Why is BF-100539 in the Medium band?"* is only answerable because the drawn
  base is now a column. Before that, the score could not be reproduced from the
  answers beside it and this question had no honest answer.

**Two corrections to what this section said.**

`http://127.0.0.1:8787/mcp` is **the wrong port**. `DEFAULT_MCP_PORT` is
**50390** (`src/config.ts`), so the address is `http://127.0.0.1:50390/mcp`
unless `gemdb.mcp.port` says otherwise.

"The server forks with the database and appears in VS Code agent mode on its
own" is **not the default**. `mcpEnabled()` reads `gemdb.mcp.enabled` with a
default of `false`, so nothing is listening until someone turns it on —
...
**`README.md` is done (2026-09-08).** It sequences CUJ-0 → 4 with
copy-pasteable commands, and every command and every line of output in it was
run against a real database on that date. House style is
`GemDB_Code/docs/demo/rabbit-in-the-hat/` (note: the path in the earlier draft
of this file was wrong — it is a directory, not `docs/demo-rabbit-in-the-hat.md`).

It opens with a **preflight** — `gemdb -c 'import re'` — because an extent
...
were reached from different code and **corroborate four of ours** — class
identity, dirtied sessions, Flask's logging stub, and `gemdb file.py` and
`sys.path`. The README cross-references it rather than restating it. We are
